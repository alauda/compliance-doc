...
2、Import the plugin using violet:
...
* `name`: Temporary name for the cluster plugin. The platform will rename it after creation based on the content, in the format `<cluster-name>-<hash of content>`, for example `global-ee98c9991ea1464aaa8054bdacbab313`.
...
You can also install the plugin by creating a ModuleInfo resource in the Alauda Container Platform global cluster:
...
metadata:
  annotations:
    cpaas.io/display-name: compliance-operator
    cpaas.io/module-name: '{"en": "Alauda Container Platform Compliance Service", "zh": "Alauda Container Platform Compliance Service"}'
  labels:
    cpaas.io/cluster-name: <cluster-name>  # target cluster name
    cpaas.io/module-name: compliance-operator
    cpaas.io/module-type: plugin
    cpaas.io/product: Platform-Center
  name: compliance-operator-<cluster-name>  # resource name
spec:
  config:
    openscapReportStorage:
      accessMode: ReadWriteOnce
      mountPath: /reports
      pvcName: compliance-openscap-reports
      size: 1Gi
      storageClass: <storage class>  # storage class for storing compliance reports
  version: v1.1.0
```