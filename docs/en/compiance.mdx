...
|-----------|------|-------|-------------|
| `spec.scanType` | string | k8s,os | Supports `platform`,`node` and `all`. In platform mode, system will only scan kubernetes resource; In node mode, system will only scan node files.|
