...
kind: Scan
metadata:
  name: scan-stig-os-micoos
  namespace: compliance-system
spec:
  profile: stig-os-micoos
  scanType: node
```
...
|-----------|------|-------|-------------|
| `spec.nodeScopeStrategy` | string | k8s, os | Scanning node selection mode. Supports `auto` and `manual` modes. In auto mode, nodes are selected based on rule-specific scope definitions. In manual mode, all rules are executed on specified nodes. |
