...
kubectl create --raw "/apis/reports.compliance.security.alauda.io/v1alpha1/namespaces/${NS}/scans/${SCAN}/export/zip" -f - <<< '{}' > ${SCAN}.zip
```
...
kubectl create --raw "/apis/reports.compliance.security.alauda.io/v1alpha1/namespaces/${NS}/scans/${SCAN}/export/zip?scanID=${SCANID}" -f - <<< '{}' > ${SCAN}-${SCANID}.zip
```
...
kubectl create --raw "/apis/reports.compliance.security.alauda.io/v1alpha1/namespaces/${NS}/scansuites/${SUITE}/export/zip" -f - <<< '{}' > ${SUITE}.zip
```
...
kubectl create --raw "/apis/reports.compliance.security.alauda.io/v1alpha1/namespaces/${NS}/scansuites/${SUITE}/export/zip?runID=${RUNID}" -f - <<< '{}' > ${SUITE}-${RUNID}.zip
```
...
curl -k -X POST \
  ...
  "${APISERVER}/apis/reports.compliance.security.alauda.io/v1alpha1/namespaces/${NS}/scans/${SCAN}/export/zip?scanID=${SCANID}"
```
...
curl -k -X POST \
  ...
  -d '{}' \
  --output "${SUITE}.zip" \
  "${APISERVER}/apis/reports.compliance.security.alauda.io/v1alpha1/namespaces/${NS}/scansuites/${SUITE}/export/zip"
```
...
- Legacy URL patterns are still supported for backward compatibility, but the `namespaces/{ns}/...` pattern is recommended for all new scripts and examples.
